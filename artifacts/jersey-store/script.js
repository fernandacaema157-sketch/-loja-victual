...
/** Called after Google OAuth redirect — reads token from URL hash and saves session */
async function handleOAuthCallback(params) {
  const { token, error } = params;
  if (error || !token) {
    navigate('#sign-in');
    toast(error === 'google_cancelled' ? 'Login cancelado.' : 'Erro ao entrar com Google.', 'error');
    return;
  }
  try {
    ...
    updateNavbar();
    navigate('#shop');
    toast(`Bem-vindo, ${user.firstName || user.email.split('@')[0]}!`);
  } catch (_) {
...
async function handleRoute() {
  ...
  try {
    switch (page) {
      ...
      case 'oauth-callback':  await handleOAuthCallback(params); return;
      case 'checkout':
