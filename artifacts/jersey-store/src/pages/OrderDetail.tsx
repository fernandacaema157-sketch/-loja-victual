...
import { ArrowLeft, Package, MapPin, CreditCard, Check, Clock, Truck, Home } from "lucide-react";
      ...
      <div className="min-h-screen bg-background"><Navbar />
        <div className="pt-16 flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <Link href="/orders" className="text-primary hover:underline">Ver meus pedidos</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background"><Navbar />
      <div className="pt-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Link href="/orders" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-6">
          ...
          {order.status !== "cancelled" && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-6 mb-6">
              <h2 className="font-bold mb-5">Rastreamento do Pedido</h2>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-card border border-border rounded-2xl p-6 mb-6">
            <h2 className="font-bold mb-4 flex items-center gap-2"><Package className="h-4 w-4 text-primary" /> Itens do Pedido</h2>
                ...
                <div key={item.id} className="flex items-center gap-3" data-testid={`order-item-${item.id}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(item.customName || item.customNumber) && (
                      <p className="text-xs text-primary font-mono">{item.customName} {item.customNumber && `#${item.customNumber}`}</p>
                    )}
                  </div>
                  <p className="font-black text-primary">R$ {(item.price * item.quantity).toFixed(2).replace(".", ",")}</p>
                </div>
              ))}
            </div>
            <div className="border-t border-border mt-4 pt-4 space-y-1.5 text-sm">
              <div className="flex justify-between text-muted-foreground"><span>Subtotal</span><span>R$ {order.subtotal.toFixed(2).replace(".", ",")}</span></div>
              <div className="flex justify-between text-muted-foreground"><span>Frete ({order.shippingMethod})</span><span>R$ {order.shippingCost.toFixed(2).replace(".", ",")}</span></div>
              <div className="flex justify-between font-black text-base pt-1 border-t border-border"><span>Total</span><span className="text-primary">R$ {order.total.toFixed(2).replace(".", ",")}</span></div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-card border border-border rounded-2xl p-5">
              <h2 className="font-bold mb-3 flex items-center gap-2 text-sm"><MapPin className="h-4 w-4 text-primary" /> Endereço de Entrega</h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card border border-border rounded-2xl p-5">
              <h2 className="font-bold mb-3 flex items-center gap-2 text-sm"><CreditCard className="h-4 w-4 text-primary" /> Pagamento</h2>
